...
Formal zaehlen wir fuer ein Wort $u$ die Anzahl der oeffnenden und schliessenden Klammern mit
$|u|_{\texttt{(}}$ bzw. $|u|_{\texttt{)}}$. Schreibe $u\preceq w$, wenn $u$ ein Praefix von $w$ ist.
Dann ist die akzeptierte Sprache
\[
\boxed{L(P)=D}
\]
mit
\[
D=\{w\in\{\texttt{(},\texttt{)}\}^*
\mid \forall u\preceq w:\ |u|_{\texttt{(}}\ge |u|_{\texttt{)}}\
\text{ und }\ |w|_{\texttt{(}}=|w|_{\texttt{)}}\}.
\]

Begruendung: Nach dem Lesen eines Praefixes $u$ liegt ueber dem Bodenmarker genau
$|u|_{\texttt{(}}-|u|_{\texttt{)}}$-mal das Symbol $1$ auf dem Stack, solange dieser Wert nicht
negativ wird. Wird er negativ, muesste der Automat bei einer schliessenden Klammer ein $1$ entfernen,
das nicht vorhanden ist; dieser Berechnungspfad bricht also ab. Der Uebergang nach $q_2$ ist genau dann
moeglich, wenn keine $1$ mehr ueber dem Bodenmarker liegen, also wenn
$|w|_{\texttt{(}}=|w|_{\texttt{)}}$ gilt.
